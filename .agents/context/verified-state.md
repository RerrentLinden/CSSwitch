...
- 2026-07-19 已纠正首次手工 DMG 重建误带 stale `CSSwitch Test.app` 的发布错误；错误附件已删除，tag 与 source 未移动。当前 DMG 从 frozen tag 的 clean build 和一次性空 staging 生成。
- 当前 DMG 的大小、SHA-256、GitHub digest、重新下载逐字节一致性、只读挂载根目录精确白名单与 arm64 executable identity 已建立；根目录发布载荷只有一个正式 `CSSwitch.app` 和 Applications 链接，不含 Test / Acceptance app。完整数值和纠正记录见 [v0.8.0 发布证据](../../docs/evidence/releases/v0.8.0.md)。
