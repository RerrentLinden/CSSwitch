...
| Kimi:联网搜索 | `tool.kimi.web_search.client-tool-bridge` | **判据是功能不是报错**:模型要拿到真实搜索结果并据此回答,且助手内容里**不出现空的 `Search results for query:`**。必须在全新会话里测。只看有没有 400 会误判——2026-08-19 一次退役尝试全程零报错,搜索却完全不工作 |
