...
但桥接仍然必需——本轮实测暴露了第二条、也更硬的缺陷:**搜索结果送不回历史**。
关掉桥接原生透传时,第 1 轮搜索成功,第 2 轮(历史带上搜索结果)必然
`400 tool_call_id  is not found`(id 为空):`web_search_tool_result` 块不携带
`tool_use_id`,Kimi 的兼容层却要求它。一次搜索让此后每一轮都失败。
