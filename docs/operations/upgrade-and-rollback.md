# CSSwitch 0.8.0 升级与回滚 / Upgrade and rollback

本说明适用于 macOS Apple Silicon 的 CSSwitch 0.8.0。0.8.0 继续复用 Science 持久化 data-dir 与外部 Skill bridge，并把 v1/v2/v3 配置安全迁移为支持多模型目录和 role bindings 的 v4；现有 API provider、active profile、Codex 配置、用户 MCP 配置和未知字段保持不变。

This guide applies to CSSwitch 0.8.0 for macOS Apple Silicon. Version 0.8.0 keeps reusing Science's persistent data-dir and external Skill bridge, then safely migrates v1/v2/v3 configuration to v4 for multi-model catalogs and role bindings. Existing API providers, the active profile, Codex configuration, user MCP entries, and unknown fields remain intact.
...
4. 确认下载文件名和目标版本是 `CSSwitch_0.8.0_aarch64.dmg` / `0.8.0`。
...
4. Confirm that the download and target version are `CSSwitch_0.8.0_aarch64.dmg` / `0.8.0`.

## 覆盖安装 / In-place install

1. 打开 DMG，把 CSSwitch 拖入「应用程序」并选择替换旧版。
2. 首次打开如果被 macOS 阻止，在 Finder 中右键 CSSwitch，选择「打开」。0.8.0 为 ad-hoc 签名且未公证；这不等于 Developer ID、notarization 或 Gatekeeper 已验证。
...
2. If macOS blocks the first launch, right-click CSSwitch in Finder and choose “Open.” Version 0.8.0 is ad-hoc signed and not notarized; this is not Developer ID, notarization, or Gatekeeper verification.
...
0.8.0 使用 v4 配置 schema，并在迁移 v1/v2/v3 时保留不可覆盖的版本备份。0.7.0 只能读取到 v3，因此不要直接拿旧 app 打开 v4 配置：应先在 0.8.0「高级设置」中使用“导出并降级到 v2”，或在所有进程退出后恢复兼容的版本备份。随后再用旧版 `.dmg` 覆盖 `/Applications/CSSwitch.app`。不要同时运行两个版本，也不要把新 sidecar 单独复制进旧版 app。回滚不会删除 Science data-dir、已安装 Skill、bundle manifest、隔离回收内容或旧 Skill Manager 数据。

Version 0.8.0 uses config schema v4 and preserves non-overwriting versioned backups while migrating v1/v2/v3. Version 0.7.0 can read only through v3, so do not open v4 configuration directly with an older app. First use “Export and downgrade to v2” under 0.8.0 Advanced Settings, or restore a compatible versioned backup after every CSSwitch process has stopped. Then replace `/Applications/CSSwitch.app` with the older DMG. Do not run two versions at once or copy a newer sidecar into an older app. Rollback does not delete the Science data-dir, installed Skills, bundle manifests, quarantined content, or legacy Skill Manager data.
